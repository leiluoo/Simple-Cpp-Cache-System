#ifndef _HASHSOLT_HPP
#define _HASHSLOT_HPP
...
int send_message(string dist_s,const char* port){
    ...
    (int)strlen(message.c_str());
    char* data = new char[message.size() + 4]; 
    int length = htonl(message.size()); 
    memcpy(data, &length, 4); 
    memcpy(data+4, message.data(), message.size()); 


    if( send(sockfd, data, strlen(message.c_str())+4, 0) < 0)
    {
...
int hashslot::send_server(){ ///向offline外的已知所有server发送最新的分布
    string dist_s = this->output() ;
    map<int,string>::iterator i;
    int out;
    for ( i = itos.begin() ;i!=itos.end();i++){
        int iii = atoi((i->second).c_str());
        out =send_message(dist_s,boost::lexical_cast<string>(iii).c_str());
    }
