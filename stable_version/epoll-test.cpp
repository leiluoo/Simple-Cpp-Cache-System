  ...
  while (1)
    {
      ...
      if(time(NULL)-time_now>5){ //每五秒检查心跳
      // printf("检查心跳\n");
        time_now = time(NULL);
        for (auto h_iter =heart.begin(),next_i = h_iter;h_iter!=heart.end();h_iter=next_i){
          ++next_i;
          if(time_now - h_iter->second>15){// 十五秒未收到心跳则server掉线
            //h_iter->first IP掉线
              ...
              while (1)
                {
                  ...
                  printf("收到消息：%s\n",message.c_str());
                  if(message.substr(0,16)=="get_distribution"){//返回分布
                    string dist_return ="distribution:"+ dist.output();
                    if( send(events[i].data.fd, dist_return.c_str(), strlen(dist_return.c_str()), 0) < 0)
                      {
                    ...
                    close(events[i].data.fd);
                    break;
                  } else if(message.substr(0,6)=="heart:"){
                    string ip = message.substr(6,4);
                    if(heart[ip]==0){
                      //上线IP
                      dist.add_server(ip);
                      dist.send_server();
                    ...
                    close(events[i].data.fd);
                    break;
                  } else if(message.substr(0,8)=="offline:"){
                      //下线IP
                      string ip = message.substr(8);
                      printf("下线IP%s\n",ip.c_str());
                      dist.del_server(ip);
                      dist.send_server();
                      heart.erase(ip);
